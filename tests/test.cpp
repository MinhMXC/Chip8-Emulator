...
class TestDisplay final : public Display {
public:
    int** screen;
...
public:
    explicit Chip8Test(Display& display, TestInputHandler& handler, Timer& timer): Chip8(display, handler, timer, false) {
    }
...
TEST_CASE("Font Loaded Correctly", "") {
    ...
    TestDisplay display{};
    Timer timer{};
    Chip8Test chip8{display, inputHandler, timer};
...
TEST_CASE("Correct Arithmetic Instruction") {
    ...
    SECTION("Correct Shift NEWER") {
        // F to make sure Y value is not important
        chip8.decodeTest(0x80F6);
        REQUIRE(chip8.getRegisters()[0] == (0x0023 >> 1));
        REQUIRE(chip8.getRegisters()[15] == 1);

        chip8.decodeTest(0x80FE);
        REQUIRE(chip8.getRegisters()[0] == (0x0023 - 1));
        REQUIRE(chip8.getRegisters()[15] == 0);
    ...
    SECTION("Correct Shift OLDER") {
        chip8.setIsOlder(true);
        chip8.decodeTest(0x8016);
        REQUIRE(chip8.getRegisters()[0] == (0x0069 >> 1));
        REQUIRE(chip8.getRegisters()[15] == 1);
...
TEST_CASE("Correct Instruction Decode", "") {
    ...
    SECTION("Correct B Instruction") {
        ...
        chip8.decodeTest(0x6001);
        chip8.decodeTest(0xB123);
        REQUIRE(chip8.getPC() == 0x0124);
    }
    ...
    SECTION("Correct D Instruction") {
        ...
        for (int i{}; i < 4; i++) {
            REQUIRE(display.screen[0][i] == 1);
        }

        for (int i{1}; i < 4; i++) {
            REQUIRE(display.screen[i][0] == 1);
            REQUIRE(display.screen[i][1] == 0);
            REQUIRE(display.screen[i][2] == 0);
            REQUIRE(display.screen[i][3] == 1);
        }

        for (int i{}; i < 4; i++) {
            REQUIRE(display.screen[4][i] == 1);
        }

        for (int i{0}; i < 4; i++) {
            for (int j{}; j < 5; j++) {
                REQUIRE(display.screen[j][i + 4] == 0);
            }
        }
        ...
        // Checking 0 has been drawn correctly
        REQUIRE(display.screen[0][62] == 1);
        REQUIRE(display.screen[0][63] == 1);

        REQUIRE(display.screen[1][62] == 1);
        REQUIRE(display.screen[1][63] == 0);

        REQUIRE(display.screen[2][62] == 1);
        REQUIRE(display.screen[2][63] == 0);

        REQUIRE(display.screen[3][62] == 0);
        REQUIRE(display.screen[3][63] == 0);

        REQUIRE(display.screen[4][62] == 0);
        REQUIRE(display.screen[4][63] == 0);
    ...
    SECTION("Correct F Instructions") {
        ...
        REQUIRE(timer.getDelayTimer() == 0x23);
        ...
        REQUIRE(timer.getSoundTimer() == 0x23);
        ...
        // FX0A
        chip8.decodeTest(0xF00A);
        REQUIRE(chip8.getRegisters()[0] == 0x23);
        REQUIRE(chip8.getPC() == 0x1FE);

        inputHandler.setKey(6);
        chip8.decodeTest(0xF00A);
        REQUIRE(chip8.getPC() == 0x1FE);
        REQUIRE(chip8.getRegisters()[0] == 6);
        ...
        chip8.setIsOlder(true);
        chip8.decodeTest(0xA003);
        chip8.decodeTest(0xF255);
        // Set Index back to original
        REQUIRE(chip8.getIndex() == 6);
        ...
        REQUIRE(chip8.getRegisters()[0] == 0);
        REQUIRE(chip8.getRegisters()[1] == 0);
        REQUIRE(chip8.getRegisters()[2] == 0);
        REQUIRE(chip8.getIndex() == 9);
    }
}